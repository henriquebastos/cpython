...
class Availability(SphinxDirective):
    ...
    def parse_platforms(self):
        ...
        if unknown := set(platforms).difference(self.known_platforms):
            cls = type(self)
...
class AuditEvent(Directive):
    ...
    def run(self):
        ...
        text = label.format(
            name=f"``{name}``", args=", ".join(f"``{a}``" for a in args if a)
        )
...
class DeprecatedRemoved(Directive):
    ...
    def run(self):
        ...
        if len(node):
            ...
            node[0].insert(0, nodes.inline('', f'{text}: ', classes=['versionmodified']))
        else:
            para = nodes.paragraph(
                '',
                '',
                nodes.inline('', f'{text}.', classes=['versionmodified']),
                translatable=False,
            )
            node.append(para)
        env = self.state.document.settings.env
